    ...
    const cargarImagen = () => {
      ...
      fetch("https://api.cloudinary.com/v1_1/ronaldimg/image/upload", {
        ...
        .then((data) => {
          ...
          setIsUploading(false); // <-- AÑADIDO: Restablecer el estado de carga
          setImageurl(data.secure_url);
        })
        .catch((err) => {
          setIsUploading(false); // <-- AÑADIDO: Restablecer también en caso de error
        ...
        <form className="signin-form" onSubmit={submitHandler} style={{ maxWidth: "700px", gap: "1.5rem" }}>
          ...
          <div className="form-section w100p">
            <details>
              <summary className="mb-1">
                <span>Existencia por Talla ({existencia} unidades)</span>
              </summary>
              <div className="flx jcenter wrap pad-0" style={{ gap: "0.5rem" }}>
                {keys.map((talla) => {
                  ...
                  return (
                    <div key={talla} className={`edit-span-tallas ${!productData.tallas[talla] ? "inactive" : ""}`}>
                      <span>{talla}</span>
                      <input
                        type="number"
          ...
          <div className="form-section w100p">
            ...
            <div className="edit-image-container mb-1 mtop-1">
              ...
              <p className="font-x centrado mtop-05">Click en el recuadro para seleccionar o cambiar la imagen</p>
            </div>
          </div>
